...
pub fn sum(v: Vec<i32>) -> i32 {
    let (lh, rh) = v.split_at(v.len() / 2);
    let (lvec, rvec) = (lh.to_owned(), rh.to_owned());

    let lsum = spawn(move || lvec.iter().sum::<i32>());
    let rsum = spawn(move || rvec.iter().sum::<i32>());

    lsum.join().unwrap() + rsum.join().unwrap()
}
